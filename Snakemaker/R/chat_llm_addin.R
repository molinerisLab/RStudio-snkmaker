...
# Save chat sessions to storage directory
save_chat_session <- function(chat_data, session_name = NULL) {
  if (is.null(session_name)) {
    session_name <- format(Sys.time(), "chat_session_%Y%m%d_%H%M%S.json")
  }

  chat_path <- get_file_path(session_name)
  writeLines(jsonlite::toJSON(chat_data, auto_unbox = TRUE, pretty = TRUE), chat_path)
}

# Load saved chat sessions
load_chat_sessions <- function() {
  chat_dir <- get_storage_dir()
  chat_files <- list.files(chat_dir, pattern = "^chat_session_.*\\.json$", full.names = TRUE)

  sessions <- list()
  for (file in chat_files) {
    tryCatch({
      content <- readLines(file, warn = FALSE)
      sessions[[basename(file)]] <- jsonlite::fromJSON(paste(content, collapse = ""))
...
chat_llm_addin <- function() {
  ...
  server <- function(input, output, session) {
    ...
    chatData <- reactiveValues(
      current = list(),
      sessions = load_chat_sessions(),  # holds previous sessions
      document_context = ""
    )
