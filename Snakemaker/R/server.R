server <- function(input, output, session) {
  ...
  # Create reactive readers for the history files with updated paths
  ...
  observe({
    r_history_path <- get_file_path("r_history.txt")
    bash_history_path <- get_file_path("bash_history.txt")

    if (file.exists(r_history_path)) {
      current_r_history(readLines(r_history_path))
    }
    if (file.exists(bash_history_path)) {
      current_bash_history(readLines(bash_history_path))
    }
  })
  ...
  classify_line_importance <- function(line, model = NULL) {
    if (is.null(model)) {
      ...
      model <- if (file.exists(selected_model_path)) {
    ...
    prompt <- paste0(
      "I have the following bash or R command: ", line,
      " It might be part of some data analysis or processing work and need to be translated into a Snakemake or Make rule, but it could also be just a one-time command from the user. For example the user might run commands as 'dir', 'cd ..' to navigate his environment and these commands don't make into the pipeline he's building. Generally, commands that for sure do not write to files are not worth making into rules. Please try to infer if this command is worth making into a rule or not. Only output: 'Important'if you classify it as Important OR 'not important'if it is a non important command."
    )
  ...
  observeEvent(input$insert, {
    ...
    if (length(selected_line) > 0L && selected_line != "") {
      writeLines(selected_line, con = get_file_path("selected_line.txt"))
      writeLines(source_flag, con = get_file_path("source_flag.txt"))
  ...
  observeEvent(input$parse_again, {
    ...
    if (length(selected_rule) > 0L && selected_rule != "") {
      writeLines(selected_rule, con = get_file_path("selected_line.txt"))
  ...
  observeEvent(input$remove_archived, {
    selected_rules <- input$archived_rules_select
    if (length(selected_rules) > 0) {
      current_archived <- archived_rules()
      # Remove the selected rules from the archive
      updated_archived <- setdiff(current_archived, selected_rules)
      archived_rules(updated_archived)

      # Update the select input
      updateSelectInput(session, "archived_rules_select", choices = updated_archived)

      # Show notification
      msg <- ifelse(length(selected_rules) > 1,
                   paste(length(selected_rules), "rules removed from archive."),
                   "Rule removed from archive.")
      showNotification(msg, duration = 2)
    } else {
      showNotification("No rules selected to remove.", type = "warning", duration = 2)
    }
  })
}
